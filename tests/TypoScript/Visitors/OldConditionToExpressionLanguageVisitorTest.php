...
final class OldConditionToExpressionLanguageVisitorTest extends TestCase
{
    ...
    public function statements(): Iterator
    {
        ...
        yield 'Hostname' => [
            'oldCondition' => '[hostname = myhost.*.com, myotherhost.*.de]',
            'newCondition' => '[like(request.getNormalizedParams().getHttpHost(), "myhost.*.com, myotherhost.*.de")]',
        ];

        yield 'IP Address' => [
            'oldCondition' => '[IP = *.*.*.123][IP = 192.168.1.34]',
            'newCondition' => '[like(request.getNormalizedParams().getRemoteAddress(), "*.*.*.123") || request.getNormalizedParams().getRemoteAddress() == "192.168.1.34"]',
        ];
